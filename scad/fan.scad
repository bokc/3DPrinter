...
module inside_bore(diameter, hub)
{
  difference()
  {
    circle(r=diameter/2,center=true,$fn=_globalResolution );
    circle(r=hub/2,center=true,$fn=_globalResolution );
  }
}
...
module mounting_holes(offset, r, h)
{
  for (i=[0:3])
  {
    rotate([0,0,i*90])
    {
      ...
      {
        circle(r=r, h=h, center=true,$fn=_globalResolution );
      }
    }
  }
}
    ...
    rotate([0,0,i*90])
    {
      translate([offset,offset])
      {
        difference()
        {
          ...
          translate([-r, -r])
          {
            circle(r=r,$fn=_globalResolution );
          }
        }
      }
    }
...
module outside_ring(width, depth, surface_thickness=3.6)
{
  difference()
  {
    cylinder(r=width*.75, h=depth-2*surface_thickness, center=true,$fn=_globalResolution );
    cylinder(r=(width+2)/2, h=depth-2*surface_thickness+0.2, center=true,$fn=_globalResolution );
  }
}
